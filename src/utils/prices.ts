...
import {Factory} from "../../generated/CvxLocker/Factory";
import {Pair} from "../../generated/CvxLocker/Pair";
...
export function getUSDRate(token: Address): BigDecimal {
    ...
    if (token != USDT_ADDRESS) {
        ...
        // log.debug("getEthRate for {} = {}", [token.toHexString(), tokenPriceETH.toString()])
        const pair = Pair.bind(SUSHISWAP_WETH_USDT_PAIR_ADDRESS);

        // log.debug("pair: {}", [pair._address.toHexString()])
        const reserves = pair.getReserves();
        ...
        const ethPriceUSD = reserve1
            ...
            .div(BIG_DECIMAL_1E6)
            .times(BIG_DECIMAL_1E18);
        // log.debug("ethPriceUSD = {}", [ethPriceUSD.toString()])
